...
#   listsgcfg <- list.files(local.path, pattern="*_solocam.cfg", full.names = TRUE)
...
#   sci.ts<-here(base.path,deployment,paste0(deployment_year, "_", deployment, "_processed-L1_", deployment, "-delayed-sci.nc"))
...
# state.to.sample.15 <- "Diving, hovering, climbing, on surface"
# sample.depth <- "1000 m"
# # add sample depth for other senors
# # sample.depth.sg <- "300 m"
...
if (processor == "GCP") {
  ...
  listma <- list.files(raw.path, deployment_year, deployment, archive-sfmc, pattern = "sample.*\\.ma$")
  # listpacfg <- list.files(raw.path, deployment_year, deployment, archive-sfmc, pattern = "*_wispr.cfg")
  # listaacfg <- list.files(raw.path, deployment_year, deployment, archive-sfmc, pattern = "*_ad2cp.cfg")
  # listaacfg <- list.files(raw.path, deployment_year, deployment, archive-sfmc, pattern = "*_azfp.cfg")
  listsgcfg <- list.files(raw.path, deployment_year, deployment, archive-sfmc, pattern = pattern="*_solocam.cfg", full.names = TRUE)
...
if (length(listma) == num_sensors) {
    for(i in 1:nrow(sensor_lookup)) {
      ...
      if (exists(f_var)) {
        ...
        full_path <- here::here(base.path, deployment, display_name[1])

        params <- parse_ma_file(full_path)
        
        baseline_rows[[f_var]] <- tibble(
          `File Name`       = display_name[1], 
          Sensor            = spec_val, 
    ...
    print(
      sensors %>%
        knitr::kable(format="markdown", align="l", booktabs=TRUE, linesep="") %>%
    ...
    for(i in 1:nrow(sensor_lookup)) {
      ...
      if (exists(f_var)) {
        ...
        if (length(matching_files) <= 1) {
          ...
          # baseline_rows[[f_var]] <- tibble(
          #   `File Name` = display_name, Sensor = spec_val, 
          #   `State to Sample` = if(params$state == "Unknown") state.to.sample.7 else params$state, 
          #   `Depth to Sample` = if(params$depth == "Unknown") sample.depth else params$depth, 
          #   `Serial Number` = sn_val
          )
        } else {
          # create original sample table where `State to Sample` and `Depth to Sample` columns are replaced with "see table x"
          baseline_rows[[f_var]] <- tibble(
          ...
          # for each sample number with more than one file, parse the filename timestamp, the `state_to_sample` value, and the `max_depth` value
          history_rows <- list()
          ...
          dynamic_tables_content[[spec_val]] <- bind_rows(history_rows) # print sensor tables after main table
        }
      }
    }
    ...
    if (length(dynamic_tables_content) > 0) {
      for (sensor_title in names(dynamic_tables_content)) {
        print(
          dynamic_tables_content[[sensor_title]] %>%
            knitr::kable(format="markdown", align="l", booktabs=TRUE, linesep="") %>%
            kableExtra::kable_styling(position="left", latex_options="striped", stripe_color="gray!15")
        )
      }
    ...
    paste("number of ma files less than number of sensors, check for missing ma files or if unused sensors are commented in above")
}
```
...
```{r}
if (processor == "GCP") {
  plot_dir <- file.path(base.path, deployment_year, deployment, "plots", "delayed", "spatialGrids-sci")
} else {
  # locally saved plots
  plot_dir <- here(base.path, deployment, "plots", "delayed", "spatialGrids-sci")
}

# Define the individual plot paths

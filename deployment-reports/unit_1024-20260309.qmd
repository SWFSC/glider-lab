...
#change file names
# set file paths to GCP
home_dir <- Sys.getenv("HOME")
base.path <- file.path(home_dir, "gcs-mnt", "swfscesd-glider-deployments-data-out")

# pull from mounted cloud buckets
eng.ts <- file.path(base.path, "2026", deployment, "processed-L1", paste0(deployment,"-delayed-eng.nc"))
raw.ts <- file.path(base.path, "2026", deployment, "processed-L0", paste0(deployment,"-delayed-raw.nc"))

# pull from local files, these are currently uploaded to gcp in cold storage
local.path <- "/home/user/glider-lab/deployment-reports/data"
listma <- list.files(local.path, pattern="sample.*\\.ma$")
# listpacfg <- list.files(local.path, pattern="*_wispr.cfg")
# listaacfg <- list.files(local.path, pattern="*_ad2cp.cfg")
# listaacfg <- list.files(local.path, pattern="*_azfp.cfg")
listsgcfg <- list.files(local.path, pattern="*_solocam.cfg")
...
The Ecosystem Science Division (ESD) at the Southwest Fisheries Science Center (SWFSC) deployed glider **glider** (serial number) on `{r} deployment.length[1]` off the coast of **location** (`{r} round(nc.eng.lat[1],digits=2)`^o^N, `{r} round(nc.eng.lon[1],digits=2)`^o^W) (Figure 1). Sensors deployed on the glider are listed in @tbl-sensors.
...
sensors %>%
  reframe(
    `File Name` = c(ctd.file,puck.file,oxy.file,sg.file),
    `Sensor` = c(ctd.spec,puck.spec,oxy.spec,sg.spec),
    `State to Sample` = c(rep("See Table 2",3),state.to.sample.7),
    #`State to Sample` = c(rep(state.to.sample.7,3),state.to.sample.15),
    `Depth to Sample` = c(rep(sample.depth,4)),
    `Serial Number` = c("9715","6866","1160","SG-V3-003")
    ) %>%
...
ca.percent.used<-(ca.used/ext.rec)*100
```
...
all.ma <- lapply(file.path(local.path, listma), readLines)
...
![a](%60%7Br%7D%20bs%60){fig-alt="A plot showing points where the glider surfaced by latitude and longitude, with two color-coded plots of backscatter by latitude and depth and by longitude and depth."} ![b](%60%7Br%7D%20chl%60){fig-alt="A plot showing points where the glider surfaced by latitude and longitude, with two color-coded plots of chlorophyll-a by latitude and depth and by longitude and depth."} ![c](%60%7Br%7D%20den%60){fig-alt="A plot showing points where the glider surfaced by latitude and longitude, with two color-coded plots of water density by latitude and depth and by longitude and depth."} ![d](%60%7Br%7D%20oxy%60){fig-alt="A plot showing points where the glider surfaced by latitude and longitude, with two color-coded plots of oxygen concentration by latitude and depth and by longitude and depth."} ![e](%60%7Br%7D%20sal%60){fig-alt="A plot showing points where the glider surfaced by latitude and longitude, with two color-coded plots of salinity by latitude and depth and by longitude and depth."} ![f](%60%7Br%7D%20temp%60){fig-alt="A plot showing points where the glider surfaced by latitude and longitude, with two color-coded plots of temperature by latitude and depth and by longitude and depth."}
